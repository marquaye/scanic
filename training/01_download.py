...
Datasets fetched:
  ...
  • FairScan        https://github.com/pynicolas/fairscan-dataset (PNG masks, CC BY-NC-SA 4.0)
...
Usage:
    python 01_download.py [--datasets all|midv500|smartdoc|fairscan|uvdoc|warpdoc|roboflow]
                          [--model] [--midv-clips N]
...
FAIRSCAN_RELEASES_API = "https://api.github.com/repos/pynicolas/fairscan-dataset/releases"


def download_fairscan():
    """Download FairScan dataset from GitHub releases (CC BY-NC-SA 4.0).

    Contains RGB images + binary PNG masks. We extract 4-corner quads from
    masks in 02_normalize.py using contour detection.
    """
    dest = RAW_DIR / "fairscan"
    if (dest / ".done").exists():
        print("[skip] FairScan already downloaded")
        return

    print("\n=== FairScan Dataset (GitHub releases, CC BY-NC-SA 4.0) ===")
    dest.mkdir(parents=True, exist_ok=True)

    resp = requests.get(FAIRSCAN_RELEASES_API, timeout=30)
    resp.raise_for_status()
    releases = resp.json()

    if not releases:
        print("  [WARN] No releases found. Trying direct clone fallback.")
        os.system(f"git clone --depth 1 https://github.com/pynicolas/fairscan-dataset {dest}")
        (dest / ".done").write_text("git-clone")
        return

    # Download all zip assets from all releases
    for release in releases:
        for asset in release.get("assets", []):
            url = asset["browser_download_url"]
            fname = dest / asset["name"]
            _download_http(url, fname, asset["name"])
            if fname.suffix == ".zip":
                _extract(fname, dest)
                fname.unlink(missing_ok=True)

    (dest / ".done").write_text("ok")
    print(f"FairScan -> {dest}")
