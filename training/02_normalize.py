...
Datasets handled:
  ...
  • fairscan   -- PNG binary masks -> contour approx -> 4-corner quad
...
def _mask_to_corners(mask_path: Path) -> list[tuple] | None:
    """Extract 4-corner quad from a binary segmentation mask via contour approx."""
...
def _parse_fairscan() -> list[dict]:
    """
    FairScan: RGB images + binary PNG masks (LabelMe-style).
    We infer corners from the largest contour in each mask.

    Expected layout (varies by release):
      data/raw/fairscan/images/<name>.jpg
      data/raw/fairscan/masks/<name>.png   (or same dir with _mask suffix)
    """
    records = []
    root = RAW_DIR / "fairscan"
    if not root.exists():
        print("[skip] FairScan not found -- run 01_download.py first")
        return records

    # Flexible: find all mask PNGs
    mask_files = list(root.rglob("*mask*.png")) + list(root.rglob("*/masks/*.png"))
    # Also consider any PNG that has a corresponding JPEG neighbour
    if not mask_files:
        for png in root.rglob("*.png"):
            jpg = png.with_suffix(".jpg")
            if jpg.exists():
                mask_files.append(png)

    print(f"FairScan: found {len(mask_files)} mask files")

    for mask_path in tqdm(mask_files, desc="fairscan"):
        # Companion image: same stem with .jpg extension
        img_path = mask_path.with_suffix(".jpg")
        if not img_path.exists():
            # Try parent/../images/ layout
            img_path = mask_path.parent.parent / "images" / (mask_path.stem.replace("_mask", "") + ".jpg")
        if not img_path.exists():
            continue

        pts = _mask_to_corners(mask_path)
        if pts is None:
            continue
        ...
        records.append({
            ...
            "height":  h,
            "source":  "fairscan",
            # One independent photo per record -> its own group.
            "group":   f"fairscan/{img_path.stem}",
            "corners": corners,
        })
